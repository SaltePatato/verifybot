  ...
  constructor(client) {
    super(client, {
     ...
     Object.defineProperty(this, "nod", {
      ...
      Object.defineProperty(this, "chosen", { value: this.nod[Math.floor(Math.random() * this.nod.length)], writable: true });
    }
    run(message) {
      if (message.channel.name === "dfchat") return super.error("You can't use that here!");
      message.channel.send(`**Head Nod** ${this.chosen}`);
      this.chosen = this.nod[Math.floor(Math.random() * this.nod.length)];
   }
};
